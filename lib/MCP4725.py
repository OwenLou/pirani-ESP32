...
MCP4725_BUS_ADDRESS = [0x60, 0x63]  # 不清楚这个0x63是什么情况
# The device supports a few power down modes on startup and during operation
MCP4725_POWER_DOWN_MODE = {'Off': 0, '1k': 1, '100k': 2, '500k': 3}

class MCP4725:
    def __init__(self, i2c, address=MCP4725_BUS_ADDRESS[0]):
        self.i2c = i2c
    ...
    def config(self, power_down='Off', value=0, eeprom=False):
        buf = bytearray()
        conf = 0x40 | (MCP4725_POWER_DOWN_MODE[power_down] << 1)
        if eeprom:
    ...
    def _powerDownKey(self, value):
        for key, item in MCP4725_POWER_DOWN_MODE.items():
            if item == value:
                return key
